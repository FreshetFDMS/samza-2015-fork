...
import org.apache.samza.task.sql.OperatorMessageCollector;
...
public class StreamSqlTask implements StreamTask, InitableTask, WindowableTask {
  ...
  public void process(IncomingMessageEnvelope envelope, MessageCollector collector, TaskCoordinator coordinator) throws Exception {
    messagesProcessed += 1;
    SqlMessageCollector msgCollector = new OperatorMessageCollector(collector, coordinator, operatorRouter);
    IncomingMessageTuple tuple = new IncomingMessageTuple(envelope);

    for (TupleOperator tupleOperator : operatorRouter.getTupleOperators(tuple.getStreamName())) {
      tupleOperator.process(tuple, msgCollector);
    }
