...
public class ProjectableFilterableStreamScanOp extends SimpleOperator implements TupleOperator {

  private ProjectableFilterableStreamScanSpec spec;
   ...
   */
  public ProjectableFilterableStreamScanOp(ProjectableFilterableStreamScanSpec spec) {
    super(spec);
    this.spec = spec;
  }
  ...
  public void process(Tuple tuple, SqlMessageCollector collector) throws Exception {
    ...
    if (filter != null && (Boolean)filter.execute(inputValues)){
      ...
      collector.send(IntermediateMessageTuple.fromTuple(tuple, false, spec.getOutputName()));
    }
  }
